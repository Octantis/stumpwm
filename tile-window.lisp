...
(defun focus-next-window (group)
  (focus-forward group (sort-windows-in group)))
...
"Pull the next hidden window into the current frame."
  (let ((group (current-group)))
    (focus-forward group (sort-windows-in group) t (lambda (w) (not (eq (frame-window (window-frame w)) w))))))

(defcommand (pull-hidden-previous tile-group) () ()
"Pull the next hidden window into the current frame."
  (let ((group (current-group)))
    (focus-forward group (nreverse (sort-windows-in group)) t (lambda (w) (not (eq (frame-window (window-frame w)) w))))))
